...
class UTMScriptingVirtualMachineImpl: NSObject, UTMScriptable {
    ...
    @objc func createSnapshot(_ command: NSScriptCommand) {
        ...
        let description = command.evaluatedArguments?["snapshotDescription"] as? String
        withScriptCommand(command) { [self] in
            guard let name = name else {
                throw ScriptingError.invalidParameter
            }
            try await UTMSnapshotService.createSnapshot(name: name, description: description, on: vm)
        }
    }

    @objc func listSnapshots(_ command: NSScriptCommand) {
        withScriptCommand(command) { [self] in
            let entries = try UTMSnapshotService.listSnapshots(on: vm)
