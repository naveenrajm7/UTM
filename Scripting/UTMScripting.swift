...
@objc public protocol UTMScriptingVirtualMachine: SBObjectProtocol, UTMScriptingGenericMethods {
    ...
    @objc optional func createSnapshotNamed(_ named: String!, objectDescription: String!) // Create a named full-VM snapshot (RAM, devices, and disk). The VM must be running or paused.
    @objc optional func listSnapshots() -> [Any] // List the names of all named snapshots for a virtual machine, newest first.
    @objc optional func restoreSnapshotNamed(_ named: String!) // Restore a virtual machine to a named snapshot.
    @objc optional func deleteSnapshotNamed(_ named: String!) // Delete a named snapshot from a virtual machine.
